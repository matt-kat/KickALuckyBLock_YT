...
let isPlatformExecutionPaused = false

function sendMessageToUnity(name, value, target = 'PlaygamaBridge') {
    if (window.unityInstance !== null) {
        window.unityInstance.SendMessage(target, name, value)
    } else {
        messageQueue.push({ name, value, target })
    }
}
...
function flushMessageQueue() {
    while (messageQueue.length > 0) {
        ...
        if (window.unityInstance !== null) {
            window.unityInstance.SendMessage(message.target, message.name, message.value)
        }
    }
}

function applyUnityExecutionPauseState() {
    const unityModule = window.unityInstance?.Module
    if (!unityModule) {
        return
    }

    if (isPlatformExecutionPaused) {
        unityModule.pauseMainLoop?.()
    } else {
        unityModule.resumeMainLoop?.()
    }
}

function onPlatformPauseStateChanged(isPaused) {
    isPlatformExecutionPaused = isPaused

    if (isPaused) {
        // Let C# synchronously save, silence audio and block input before rendering stops.
        sendMessageToUnity('OnPauseStateChanged', 'true')
        applyUnityExecutionPauseState()
        return
    }

    // The Unity loop must be running before C# can process the resume callback.
    applyUnityExecutionPauseState()
    sendMessageToUnity('OnPauseStateChanged', 'false')
}

function sendBridgeLayoutStateToUnity() {
    const safeArea = bridge.device.safeArea || {
        top: 0,
        bottom: 0,
        left: 0,
        right: 0,
    ...
    sendMessageToUnity(
        'OnBridgeLayoutStateChanged',
        JSON.stringify({
            orientation: bridge.device.orientation || '',
            width: window.innerWidth,
            height: window.innerHeight,
            deviceType: bridge.device.type || 'unknown',
            safeArea,
        }),
        'ScreenOrientationLayoutBootstrapper')
}
...
async function loadSingleEntryZip(url, mimeType) {
    ...
    if (compressionMethod === 0) {
        ...
        throw new Error(`Unsupported ZIP compression method ${compressionMethod} in ${url}`)
    }

    if (uncompressedData.byteLength !== uncompressedSize) {
        throw new Error(
            `Invalid decompressed size for ${url}: expected ${uncompressedSize}, got ${uncompressedData.byteLength}`)
    }
...
const isYouTubeRuntime =
    window.location.hostname.includes('usercontent.goog') ||
    new URLSearchParams(window.location.search).get('platform_id') === 'youtube'
...
if (isYouTubeRuntime) {
    ...
    bridgeScript.nonce = document.querySelector('script[nonce]')?.nonce || ''
...
function initializeBridge() {
    ...
    bridge
        ...
        .then(() => {
            ...
            bridge.platform.on('pause_state_changed', onPlatformPauseStateChanged)
            bridge.device.on('orientation_state_changed', sendBridgeLayoutStateToUnity)
            bridge.device.on('screen_size_changed', sendBridgeLayoutStateToUnity)
            sendBridgeLayoutStateToUnity()

            let unityLoader = document.createElement('script')
            unityLoader.src = 'Build/KickALuckyBLock_YT.loader.js'
            unityLoader.nonce = document.querySelector('script[nonce]')?.nonce || ''
            unityLoader.onload = async () => {
                let dataBlobUrl = null
                let codeBlobUrl = null

                try {
                    [dataBlobUrl, codeBlobUrl] = await Promise.all([
                        loadSingleEntryZip('Build/KickALuckyBLock_YT.data.unityweb.zip', 'application/octet-stream'),
                        loadSingleEntryZip('Build/KickALuckyBLock_YT.wasm.unityweb.zip', 'application/wasm'),
                    ])
                    ...
                    const unityInstance = await createUnityInstance(
                        ...
                        {
                            dataUrl: dataBlobUrl,
                            frameworkUrl: 'Build/KickALuckyBLock_YT.framework.js.unityweb',
                            codeUrl: codeBlobUrl,
                            streamingAssetsUrl: 'StreamingAssets',
                            companyName: 'Matt.car.crash.simulator',
                            productName: 'Car Crash Test Simulator',
...
window.achievementsGetAchievements = function() {
    bridge.achievements.getAchievements()
        .then(data => {
            if (data) {
                sendMessageToUnity('OnAchievementsGetAchievementsCompletedSuccess', JSON.stringify(data))
            } else {
                sendMessageToUnity('OnAchievementsGetAchievementsCompletedSuccess', '')
            }
        })
        .catch(error => {
            sendMessageToUnity('OnAchievementsGetAchievementsCompletedFailed', 'false')
        })
}
